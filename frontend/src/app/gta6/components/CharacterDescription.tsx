    ...
    >
        ...
        {officialDesc.map((paragraph, index) => {
          ...
          return (
            <motion.div 
              ...
              className={`
                ${renderAsQuote ? 'my-4' : 'text-muted-foreground'}
              `}
            >
              {renderAsQuote ? (
                <div className="relative pl-6">
                  <Quote className="absolute left-0 top-0 text-primary/30 h-5 w-5" />
                  <p className="text-base sm:text-lg font-medium text-white/90 italic leading-relaxed">
      ...
      {officialDesc.length > 0 && (
        <motion.p 
          ...
          className="text-base font-medium text-white/90 mt-6 italic"
        >
      ...
      <div className="mt-6 h-0.5 w-full bg-gradient-to-r from-white/10 via-white/20 to-transparent"></div>
    </motion.div>
  );
} 